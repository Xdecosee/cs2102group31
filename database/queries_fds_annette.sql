...
SELECT DR.uid, EXTRACT(YEAR FROM O.date) as year, EXTRACT(MONTH FROM O.date) as month, count(D.rating) as totalRatings
...
SELECT DR.uid, EXTRACT(YEAR FROM O.date) as year, EXTRACT(MONTH FROM O.date) as month, avg(D.rating) as avgRatings
FROM DeliveryRiders DR
LEFT JOIN Delivers D on DR.uid = D.uid
LEFT JOIN Orders O on D.orderID = O.orderID
GROUP BY DR.uid, EXTRACT(YEAR FROM O.date), EXTRACT(MONTH FROM O.date);
...
SELECT DR.uid, EXTRACT(YEAR FROM O.date) as year, EXTRACT(MONTH FROM O.date) as month, avg(O.deliveryDuration) as avgDuration
FROM DeliveryRiders DR
LEFT JOIN Delivers D on DR.uid = D.uid
LEFT JOIN Orders O on D.orderID = O.orderID
GROUP BY DR.uid, EXTRACT(YEAR FROM O.date), EXTRACT(MONTH FROM O.date);
...
SELECT location, date, EXTRACT(HOUR FROM timeOrderPlace) AS hour, count(*) AS totalOrders 
FROM Orders 
...
/*FULL TIME*/
SELECT F.uid, EXTRACT(YEAR FROM WW.workDate) as year, EXTRACT(MONTH FROM WW.workDate) as month, SUM(WW.numCompleted) as totalOrders
FROM FullTime F
INNER JOIN WorkingWeeks WW ON F.uid = WW.uid
GROUP BY F.uid, EXTRACT(YEAR FROM WW.workDate), EXTRACT(MONTH FROM WW.workDate)
UNION
/*PART TIME*/
SELECT P.uid, EXTRACT(YEAR FROM WD.workDate) as year, EXTRACT(MONTH FROM WD.workDate) as month, SUM(WD.numCompleted) as totalOrders
FROM PartTime P
...
/*FULL TIME*/
SELECT F.uid, EXTRACT(YEAR FROM WW.workDate) as year, EXTRACT(MONTH FROM WW.workDate) as month, count(shiftID) * 8 as totalHours
FROM FullTime F
INNER JOIN WorkingWeeks WW on F.uid = WW.uid
...
UNION
/*PART TIME*/
SELECT P.uid, EXTRACT(YEAR FROM WD.workDate) as year, EXTRACT(MONTH FROM WD.workDate) as month, 
