...
FROM foodItemsSold
-- WHERE MONTH = 4 requires user input??
FETCH FIRST 5 ROW ONLY;

/** view the number of orders received for each promotion **/
SELECT EXTRACT(MONTH FROM (date)) AS month, promoID, COUNT (distinct orderID) as num
FROM FromMenu natural join Orders
WHERE orderStatus = 'Completed'
GROUP BY promoID, EXTRACT(MONTH FROM (date));
