...
sql.query = {
    ...
    riderSummary: 'With reviewInfo as ('+
    'SELECT distinct DR.uid, EXTRACT(Year FROM (O.date)) AS year, EXTRACT(Month FROM (O.date)) as month, count(D.rating) as totalRatings, ROUND(avg(D.rating),1) as avgRatings, '+ 
    ...
    'WHERE EXTRACT(Year FROM (O.date)) = $1 AND EXTRACT(Month FROM (O.date)) = $2 '+
        'GROUP BY DR.uid, year, month), ' +
            'totalOrdersDelivered as (' +
            'SELECT distinct F.uid, EXTRACT(YEAR FROM WW.workDate) as year, ' +
            'EXTRACT(Month FROM (WW.workdate)) as month, SUM(WW.numCompleted) as totalOrders ' +
            'FROM FullTime F INNER JOIN WorkingWeeks WW ON F.uid = WW.uid ' +
            'WHERE EXTRACT(Year FROM (WW.workdate)) = $1 AND EXTRACT(Month FROM (WW.workdate)) = $2 ' +
            'GROUP BY F.uid,year, month ' +
            'UNION ' +
            ...
            'FROM PartTime P INNER JOIN WorkingDays WD ON P.uid = WD.uid ' +
            'WHERE EXTRACT(Year FROM (WD.workdate)) = $1 AND EXTRACT(Month FROM (WD.workdate)) = $2 ' +
            'GROUP BY P.uid, year,month), ' + 
            'hoursInfo as (SELECT distinct F.uid, EXTRACT(YEAR FROM WW.workDate) as year, EXTRACT(Month FROM (WW.workdate)) as month, ROUND(count(shiftID) * 8,0) as totalHours ' +
            'FROM FullTime F ' +
            'INNER JOIN WorkingWeeks WW on F.uid = WW.uid ' +
            'WHERE (WW.numCompleted > 0)  ' +
            'AND EXTRACT(Year FROM (WW.workdate)) = $1  AND EXTRACT(Month FROM (WW.workdate)) = $2 ' +
            'GROUP BY F.uid, year, month  ' +
            'UNION ' +
            'SELECT distinct P.uid, EXTRACT(YEAR FROM WD.workDate) as year, EXTRACT(Month FROM (WD.workdate)) as month, ' +
            'ROUND(sum(DATE_PART(\'hour\', WD.intervalEnd-WD.intervalStart) * 60 + DATE_PART(\'minute\', (WD.intervalEnd-WD.intervalStart)))::decimal / 60,0) as totalHours '+
            'FROM PartTime P INNER JOIN WorkingDays WD on P.uid = WD.uid ' +
            'WHERE (WD.numCompleted > 0) ' +
            'AND EXTRACT(Year FROM (WD.workdate)) = $1 AND EXTRACT(Month FROM (WD.workdate)) = $2 ' +
            'GROUP BY P.uid, year, month) ' +
            'Select * From totalOrdersDelivered natural join reviewInfo natural join hoursInfo natural join driversalary ',
